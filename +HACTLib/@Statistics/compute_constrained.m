function compute_constrained(obj)
	...
	by_interp = HACTLib.computation.KernelSmoother(...
		tmp(:,1), tmp(:,2), 0.3, kernel_options);
	
	obj.liqw_lt_ysixth = obj.sfill(by_interp.keval(1/6), 'b_i <= y_i / 6');
   	obj.liqw_lt_ytwelfth = obj.sfill(by_interp.keval(1/12), 'b_i <= y_i / 12');
	...
	wy_interp = HACTLib.computation.KernelSmoother(...
		tmp(:,1), tmp(:,2), 0.3, kernel_options);
	
	obj.w_lt_ysixth = obj.sfill(wy_interp.keval(1/6), 'w_i <= y_i / 6', 2);
	obj.w_lt_ytwelfth = obj.sfill(wy_interp.keval(1/12), 'w_i <= y_i / 12', 2);
