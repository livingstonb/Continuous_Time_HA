classdef GridConstruction
	methods (Static)
		...
		function gridpos = create_positive_asset_grid(vmax, curv, npts, varargin)
			gridpos = HACTLib.model_objects.GridConstruction.create_curved_grid(0, vmax, curv, npts, varargin{:});
		end
		...
		function gridneg = create_negative_asset_grid(vmin, curv, npts, varargin)
			...
			gridneg1 = HACTLib.model_objects.GridConstruction.create_curved_grid(vmin, mid_neg, curv, npts_neg1, varargin{:});

			% part of grid close to zero
			gridneg2 = HACTLib.model_objects.GridConstruction.create_curved_grid(mid_neg, 0, curv, npts_neg2, varargin{:}, 'flip', true);
