classdef HJBSolverSDU < HACTLib.computation.HJBSolver
	...
	methods
		function obj = HJBSolverSDU(p, income, varargin)
			obj = obj@HACTLib.computation.HJBSolver(p, income, varargin{:});
		end
	...
	methods (Access=protected)
		...
		function [inctrans, inctrans_k] = get_implicit_explicit_inctrans(obj, k)
			inctrans = HACTLib.aux.sparse_diags(obj.sdu_adj(:,k,k), 0);
		...
		function Vn1_k = update_Vk_implicit_explicit(...
			...
			Vn1_k = update_Vk_implicit_explicit@HACTLib.computation.HJBSolver(...
				obj, V_k, u_k, k, Bk, inctrans_k);
		end
	end
end