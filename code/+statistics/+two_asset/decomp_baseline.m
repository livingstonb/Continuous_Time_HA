function decomp = decomp_baseline(s0,s1)
    ...
    if p1.nz > 1
        ...
        m1 = reshape(m1,[],p1.nz);
        ptmass1 = reshape(ptmass1,[],p1.nz);
        
        m1 = sum(m1 .* ptmass1,2) ./ sum(ptmass1,2);
        m1 = m1(:);
        m1(sum(ptmass1,2)<1e-7) = 0;
        ptmass1 = sum(ptmass1,2);
    end
    ...
    decomp.term1 = (m1 - m0)' * ptmass0; 
    decomp.term2 = m0' * (ptmass1 - ptmass0);
    decomp.term3 = (m1 - m0)' * (ptmass1 - ptmass0);
    ...
    for ia = 1:numel(p0.decomp_thresholds)
        ...
        if p0.OneAsset == 1
        	idx = assets0 <= abar;
        	% HtM households
        	decomp.term2a(ia) = m0(idx)' * (ptmass1(idx) - ptmass0(idx));
        	% Non-HtM households
        	decomp.term2b(ia) = m0(~idx)' * (ptmass1(~idx) - ptmass0(~idx));
            
            decomp.term2c(ia) = NaN;
        else
            loc_zero = (grdKFE0.a.matrix(:)<=abar) & (grdKFE0.b.matrix(:)<=abar);
            decomp.term2a(ia) = m0(loc_zero)' * (ptmass1(loc_zero) - ptmass0(loc_zero));
            ...
            decomp.term2c(ia) = m0(bpos)' * (ptmass1(bpos) - ptmass0(bpos));
        end
    end
end